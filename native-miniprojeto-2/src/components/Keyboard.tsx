  ...
  const getResult = () => {
    switch (operation) {
      case "+":
        clear();
        setResult(
          String(parseInt(secondNumber) + parseInt(firstNumber)).slice(0, 4)
        );
        break;
      case "-":
        clear();
      ...
      case "/":
        ...
        break;
      case "cos":
        const rad = (parseInt(firstNumber) * Math.PI) / 180;
        setResult(String(Math.cos(rad)).slice(0, 4));
        break;
      case "SEN":
        const radSen = (parseInt(firstNumber) * Math.PI) / 180;
        setResult(String(Math.sin(radSen)).slice(0, 4));
        break;
      case "tan":
        const radTan = (parseInt(firstNumber) * Math.PI) / 180;
        setResult(String(Math.tan(radTan)).slice(0, 4));
        break;
      default:
        clear();
        setResult(0);
    ...
    <View style={styles.viewBottom}>
      ...
      <View style={styles.row}>
        <Button
          isGrey
          title="COS"
          onPress={() => handleOperationPressed("cos")}
        />
        <Button
          isGrey
          title="SEN"
          onPress={() => handleOperationPressed("SEN")}
        />
        <Button
          isGrey
          title="TAN"
          onPress={() => handleOperationPressed("tan")}
        />
        <Button
          title="÷"
          onPress={() => handleOperationPressed("/")}
